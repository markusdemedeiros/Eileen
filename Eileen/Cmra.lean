...
/-- An element cx is an -/
def is_maximal_iidempotent_lb [IRel α] [CommSemigroup α] (x : α) (n : ℕ) (cx : α)  : Prop :=
  is_iidempotent_lb x n cx ∧ ∀ m y, m ≤ n -> is_iidempotent_lb x m y -> y ≲[m] cx

def no_maximal_iidempotent [IRel α] [CommSemigroup α] (x : α) : Prop :=
...
class CMRA (α : Type*) extends OFE α, CommSemigroup α, Valid α, IValid α where
  -- Q: why not nonexpansive2?
  op_nonexpansive (x : α) : nonexpansive (op x)
...
lemma valid_of_forall_validN [CMRA α] (x : α) : (∀ n, ✓[n] x) -> ✓ x := by
  apply (valid_iff_forall_validN _).mpr

lemma validN_of_valid [CMRA α] (x : α) : ✓ x -> (∀ n, ✓[n] x) := by
